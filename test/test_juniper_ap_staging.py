...
class TestValidateIfSiteDefinedOnCSVExistsInMist(unittest.TestCase):
    ...
    def test_all_sites_exist(self):
        mist_sites_configs = [{'name': 'SiteA'}, {'name': 'SiteB'}, {'name': 'SiteC'}]
        ap_csv = [{'Site Name': 'SiteA'}, {'Site Name': 'SiteB'}, {'Site Name': 'SiteC'}]
        try:
            validate_if_site_defined_on_csv_exists_in_mist(mist_sites_configs, ap_csv)
        except ValueError:
            self.fail("validate_if_site_defined_on_csv_exists_in_mist() raised ValueError unexpectedly!")
    ...
    def test_site_not_exist(self):
        ...
        with self.assertRaises(ValueError) as cm:
            validate_if_site_defined_on_csv_exists_in_mist(mist_sites_configs, ap_csv)
        self.assertEqual(str(cm.exception), "Site name 'SiteD' found in CSV file but not in mist site configurations.")
    ...
    def test_empty_mist_sites_configs(self):
        ...
        with self.assertRaises(ValueError) as cm:
            validate_if_site_defined_on_csv_exists_in_mist(mist_sites_configs, ap_csv)
        self.assertEqual(str(cm.exception), "Site name 'SiteA' found in CSV file but not in mist site configurations.")
...
class TestValidateAP(unittest.TestCase):
    ...
    def test_all_aps_exist(self):
        mist_inventory = [{'type': 'ap', 'mac': 'aabbccddeeff'}, {'type': 'ap', 'mac': '112233445566'}]
        ap_csv = [{'MAC Address': 'aa:bb:cc:dd:ee:ff'}, {'MAC Address': '11:22:33:44:55:66'}]
        try:
            validate_if_ap_defined_on_csv_exists_in_mist(mist_inventory, ap_csv)
        except ValueError:
            self.fail("validate_if_ap_defined_on_csv_exists_in_mist raised ValueError unexpectedly!")
    ...
    def test_mixed_inventory(self):
        mist_inventory = [{'type': 'ap', 'mac': 'aabbccddeeff'}, {'type': 'switch', 'mac': '112233445566'}]
